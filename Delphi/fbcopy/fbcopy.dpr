...
function EnviaEMailComAnexo(
  ...
  ASSL: Boolean): Boolean; external 'libxnet.dll' name 'Zeos_EnviaEMailComAnexo';
...
function CheckParam(ANomeParam: string): Boolean;
var
...
begin
  ...
  begin
    ...
    if ((Length(SParam) = LenNomeParam) and (SParam = ANomeParam)) or
       (Copy(SParam, 1, Pos('=', SParam)-1) = ANomeParam) then
    begin
      Result:= True;
      Exit;
    end;
...
function GetValorParam(ANomeParam: string): string;
var
  LenNomeParam, i: Integer;
    SParam: string;
begin
...
begin
  ...
  DeleteFile(PAnsiChar(NomeArqLog));
  ...
  Write('Servidor PoliDados Firebird realizando backup, aguarde');
    ...
    WinExec(PAnsiChar(cmdline), SW_HIDE);
    i:= 0; j:= 0;
    Write('.');
    Sleep(2000);
    Write('.');
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            DeleteFile(PAnsiChar(ArqZip));
            if MoveFile(PAnsiChar(ArqZipTmp), PAnsiChar(ArqZip)) then
              WriteLn(' OK ')
            else
            begin
              WriteLn(' FALHOU ');
        ...
        begin
          DeleteFile(PAnsiChar(ArqBackup));
          if MoveFile(PAnsiChar(ArqBackupTmp), PAnsiChar(ArqBackup)) then
            WriteLn(' OK ')
          else
          begin
            WriteLn(' FALHOU ');
    ...
    begin
      ...
      begin
        ...
        WinExec(PAnsiChar(ArqBat), SW_SHOW);
      end;
    end
    ...
    begin
      ...
      try
        ...
        if FPastaBackup1 <> '' then
          CopyFile(PAnsiChar(ArqBackup), PAnsiChar(FPastaBackup1 + '\' + NomeArqDestMultiplexa), False);
        if FPastaBackup2 <> '' then
          CopyFile(PAnsiChar(ArqBackup), PAnsiChar(FPastaBackup2 + '\' + NomeArqDestMultiplexa), False);
        if FPastaBackup3 <> '' then
          CopyFile(PAnsiChar(ArqBackup), PAnsiChar(FPastaBackup3 + '\' + NomeArqDestMultiplexa), False);
        if FPastaBackup4 <> '' then
          CopyFile(PAnsiChar(ArqBackup), PAnsiChar(FPastaBackup4 + '\' + NomeArqDestMultiplexa), False);
        if FPastaBackup5 <> '' then
          CopyFile(PAnsiChar(ArqBackup), PAnsiChar(FPastaBackup5 + '\' + NomeArqDestMultiplexa), False);
    ...
    begin
      ...
      if EnviaEMailComAnexo(PAnsiChar('smtp.gmail.com'), PAnsiChar('polidados@gmail.com'),
        PAnsiChar('*000222*'), 465, PAnsiChar('polidados@gmail.com'),
        PAnsiChar('poligestor@polidados.com'),
        PAnsiChar('Envio de Email via FbCopy'), PAnsiChar(''),
        PAnsiChar(ArqBackup), '', True) then
          Write(' OK ')
      else
        Write(' FALHOU');
      Sleep(5000);
...
var
  copia: TCopia;
begin
  WriteLn('fbcopy V18 - Utilitario para copia de bancos de dados Firebird');
  WriteLn('PoliDados Informatica Ltda');
  ...
  try
    DeleteFile(PAnsiChar(ExtractFilePath(ParamStr(0)) + 'fbexit.dat'));
    copia:= TCopia.Create;
    try
      copia.Run;
    finally
