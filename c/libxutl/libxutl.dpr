...
function _Zeos_Condicao(AQuery: TDataSet; const APrefixo: string = ''): string;
var
  fldID: TField;
  IdAtual: Integer;
  sep: string;
...
{@
  Formata uma condição do SQL de um DataSet através do campo id.
  Parâmetros:
    AQuery - dataset
    APrefixo - prefixo a ser adicionado
    ACondicaoBuffer - buffer para retorno da condição
    ACondicaoSize - tamanho do buffer

  Autor: Everton de Vargas Agilar
}
procedure Zeos_Condicao(AQuery: TDataSet; const APrefixo: PAnsiChar;
  out ACondicaoBuffer: PAnsiChar; ACondicaoSize: Integer);
var
  condicao: string;
begin
  condicao:= _Zeos_Condicao(AQuery, APrefixo);
  StrPLCopy(ACondicaoBuffer, condicao, ACondicaoSize)
end;
...
function ZeosSQLResult(const ASQL: PAnsiChar; AConnection: TZConnection): Boolean;
var
  qry: TZReadOnlyQuery;
begin
...
begin
  lpLength:= 250;
  if WNetGetConnection(PAnsiChar(ALetra), lpRemoteName, lpLength) = NO_ERROR then
...
begin
  Result:= WNetCancelConnection(PAnsiChar(ALetra), True) = NO_ERROR;
end;
...
begin
  Result:= WNetAddConnection(PAnsiChar(ACaminho), nil, PAnsiChar(ALetra)) = NO_ERROR;
  if Result then
    AErrorFlag:= 0
  else
    AErrorFlag:= GetLastError;
...
begin
  ...
  begin
    ...
    begin
      ...
      Exit;
      { Erro no AutoGiro
        parâmetro S: VWSAVEIRO1.6 CAMINHONETE.ALCOOL-03/03-100CV-PARTIC-BRANCA-PL-ILE1384-CH-9BWE805X634006085-REN-80327719-9
        Comentado em 20/04/2010
        Este comentário pode ser apagado da próxima vez.
...
begin
  ...
  GetTempFileName(Dir, PAnsiChar('ZRP'), 0, Nome);
  Result := StrPas(Nome);
end;
...
begin
  ...
  Windows.GetTempPath(Length(Result), PAnsiChar(Result));
...
begin
  SetLength(Result, 255);
  GetWindowsDirectory(PAnsiChar(Result), 255);
  Result:= StrPas(PAnsiChar(Result));
end;
...
begin
  SetLength(Result, 255);
  GetCurrentDirectory(255, PAnsiChar(Result));
  Result:= StrPas(PAnsiChar(Result));
end;
...
const
   ...
   ComAcento = ' ‚¡¢£µÖàéƒˆ“¶ÒâÆä¤Çå¥„”Ž™š…·‡€¦§ýŒ×–ê‰‹ØŠÔÞ•ã—ëàâêôûãõáéíóúçüÀÂÊÔÛÃÕÁÉÍÓÚÇÜºª°€';
...
begin
  ...
  try
    ...
    begin
      ...
      if not SHGetPathFromIDList(ppidl, PAnsiChar(Result)) then
        raise exception.create('SHGetPathFromIDList failed : invalid pidl');
      SetLength(Result, lStrLen(PAnsiChar(Result)));
    end;
  finally
   if ppidl <> nil then
...
begin
  try
    GetVolumeInformation(PAnsiChar(ADrive+':\'),
...
function Zeos_ValidaEMail(const EMailIn: PAnsiChar):Boolean;
const
...
begin
  ...
  with ISLink do
  begin
    SetPath(PAnsiChar(TargetName));
    SetWorkingDirectory(PAnsiChar(ExtractFilePath(TargetName)));
  end;
...
var
   ...
   Data: Pointer;
begin
   Size := GetFileVersionInfoSize(PAnsiChar(ANomePacote), lpHandle);
   Data := AllocMem(Size);
   GetFileVersionInfo(PAnsiChar(ANomePacote), lpHandle, Size, Data);
   ...
   { Lê o valor de chave }
   VerQueryValue(Data, PAnsiChar('\\StringFileInfo\' +
...
begin
   case reason of
     ...
     begin
       ...
       if Date > StrToDate('30/03/2010')+550 then
         ExitCode:= -1;
